  ...
  foreach ($schema as $fieldname => $fieldSchema) {
    ...
    if (@$fieldSchema['type'] == 'date') {
      ...
      if ($fieldSchema['showTime']) {
        ...
        if ($fieldSchema['showSeconds'])     { $requiredDateSuffixes[] = 'sec'; }
      }
    ...
    elseif (@$fieldSchema['type'] == 'textbox' && @$fieldSchema['autoFormat']) {
      ...
      $textLength = mb_strlen((string)$value);
    }
    ...
    if ($isMyAccountPasswordField && $newPasswordEntered && !$errors) {
      $_REQUEST['password:old'] = trim(@$_REQUEST['password:old']); // v2.52 remove leading and trailing whitespace
