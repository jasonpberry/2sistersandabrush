...
function getRecords($options) {
  ...
  $options['limit']   = intval(@$options['perPage']) ? $options['perPage'] : intval(@$options['limit']);
  ...
  if ($options['offset'] && !$options['limit']) { $options['limit'] = 1000000; } // if offset and no limit set, limit to high number: suggested by per MySQL docs
  ...
  if (@$_REQUEST['preview:menu'] == @$options['tableName'] && ($isSingleMenuWithBlankWhere || $hasWhereWithPreviewNum)) { // checking for 999... prevent an infinite loop
    return _getRecords_preview($schema, $options);
  }

  // get query
  $query = _getRecords_getQuery($options, $schema);
  $countQuery = _getRecords_getCountQuery($options, $schema);

  // load from cache
  if (!empty($options['useCache'])) {
    ...
    if ($results) {
      [$rows, $listDetails, $schema] = $results;
  ...
  // Get records
  [$rows, $totalRecords] = _getRecords_loadResults($query, $countQuery, $options, $schema);

  // Add _tableName key
  foreach ($rows as $key => $record) { $rows[$key]['_tableName'] = $options['tableName']; }

  // Add pseudo-fields
  if (!empty($options['loadPseudoFields'])) { _getRecords_addPseudoFields($rows, $options, $schema); }

  // Add uploads
  if (!empty($options['loadUploads'])) { _getRecords_addUploadFields($rows, $options, $schema); }

  // Add createdBy.fields to records
  // // v2.50 set fields for alternate accounts table, eg: otherTable.username
  if (!empty($options['loadCreatedBy']) && !empty($schema['createdByUserNum'])) {
  ...
  if (!empty($options['joinTable'])) { _getRecords_joinTable($rows, $options); }
  ...
  // save to cache
    if (!empty($options['useCache']) && function_exists('turboCache_save')) {
        turboCache_save($options['tableName'], $query, [$rows, $listDetails, $schema]);
    }
...
// getRecordsCustom() - return data in the same format as getRecords but with custom SQL query
// ... this is an easy way to get paging working
//  list($recordList, $listMetadata) = getRecordsCustom(array(
//    'query'   => "SELECT * FROM tablename",
//    'perPage' => 5,
//    'pageNum' => 4, // defaults to $_REQUEST['page'] if not specified
//  ));
function getRecordsCustom($options) {
  ...
  if (!preg_match("/SQL_CALC_FOUND_ROWS/i", $options['query'])) { // add SQL_CALC_FOUND_ROWS if needed
    $options['query'] = preg_replace("/^\s*SELECT\b/i", "SELECT SQL_CALC_FOUND_ROWS", $options['query']);
...
function _getRecords_preview($schemaIn, $options)
{
    ...
    if ($productionRecord) {
        ...
        $record = _addUndefinedDefaultsToNewRecord(
            $record,
            getMySqlColsAndType(mysql_escape(getTableNameWithPrefix($options['tableName'])))
        );
    }

    // if there was no production record available, default some fields
    if (!empty($schema['updatedByUserNum'])) {
        $record['updatedByUserNum'] = $CURRENT_USER['num'];
    }
    if (!empty($schema['updatedDate'])) {
  ...
  elseif(@$options['useSeoUrls'])  { $record['_link'] = PREFIX_URL.@$schema['_detailPage'] . '/' . $filenameValue . $record['num'] . "/"; }
    ...
    // Add pseudo-fields
    if (!empty($options['loadPseudoFields'])) {
        _getRecords_addPseudoFields($rows, $options, $schema);
    }

    // Add uploads
    if (!empty($options['loadUploads'])) {
    ...
    return [$rows, $listDetails, $schema];
}


//
function _getRecords_errorChecking($options): void
{
  global $VIEWER_NAME;
...
//
function _getRecords_getQuery($options, $schema): string
{
    global $TABLE_PREFIX;

    // create fieldlist
    $tableAlias   = $options['tableName']; // doesn't get $TABLE_PREFIX added by mysql_decrypt_addSelectExpr
    $selectFields = "`{$options['tableName']}`.*".mysql_decrypt_addSelectExpr(
            $options['tableName'],
            $schema,
            $tableAlias
        );
    ...
    if (@$options['leftJoin']) {
        ...
        // add qualified fieldnames to schema
        ...
        foreach ($options['leftJoin'] as $foreignTable => $foreignKey) {
            ...
            */
            ...
            foreach (array_keys($foreignSchemaFields) as $fieldname) {
                if (in_array(@$foreignSchemaFields[$fieldname]['type'], $validFieldTypes) || in_array(
                        $fieldname,
                        $validFieldNames
                    )) {
                    $isEncrypted  = !empty($foreignSchemaFields[$fieldname]['isEncrypted']);
                    $selectFields .= ",\n";
                    $selectFields .= $isEncrypted ? _mysql_decrypt_columnExpr(
    ...
    if (!empty($schema['createdByUserNum']) && !empty($schema['_hideRecordsFromDisabledAccounts']) && empty($options['includeDisabledAccounts'])) {
        if ($where) {
            $where .= " AND ";
        }
    ...
    // adds WHERE to the beginning of string, do this LAST
    if (@$options['orWhere']) {
    ...
    // add select expr
    if (!empty($options['addSelectExpr'])) {
...
//
function _getRecords_getCountQuery($options, $schema): string
{
    global $TABLE_PREFIX;
    ...
    $where = _addWhereConditionsForSpecialFields($schema, $where, $options, $options['tableName']); // adds WHERE to the beginning of string, do this LAST
    if (@$options['orWhere']) {
        $where = preg_replace("/^\s*WHERE\s*/i", '', $where); // remove WHERE keyword
        if ($where) {
            $where = "($where) OR {$options['orWhere']}";
...
//
function _getRecords_loadResults($query, $countQuery, $options, $schema): array
{
    global $VIEWER_NAME;

  // execute query

    $result = mysqli()->query($query) or dieAsCaller(trim(@$VIEWER_NAME." MySQL Error: ".htmlencode(mysqli()->error)));
  $records   = [];
  while ($record = $result->fetch_assoc()) {
      ...
      elseif(!empty($options['useSeoUrls'])) { $record['_link'] = PREFIX_URL.@$schema['_detailPage'] . '/' . $filenameValue . $record['num'] . "/"; }
      else                                   { $record['_link'] = PREFIX_URL.@$schema['_detailPage'] . '?' . $filenameValue . $record['num']; }
...
function __replaceUnderscoresInRequest($tablename): void
{
...
//
function _getRecords_joinTable(&$rows, $options, $joinTable = ''): void
{
  $joinTable  = $joinTable ?: $options['joinTable'];
  ...
  // load rows
  [$joinrows,,$schema] = getRecords(array(
    'tableName'               => $joinTable,
...
//
function _getRecords_addPseudoFields(&$records, $options, $schema): void
{
  if (!$records) { return; }
...
//
function _getRecords_addUploadFields(&$rows, $options, $schema, $preSaveTempId = null): void
{
    if (empty($options['loadUploads'])) {
        return;
    }
...
//
function _getRecords_getListDetails($options, $rowCount, $totalRecords, $schema): array
{
  ### get list details
  $details = [];
...
//
function _getOrderByFromUrl($schema): ?string
{
...
function getCategories($options) {
  ...
  //
  [$categoryRecords] = getRecords(array(
    'tableName'       => $options['tableName'],
    'loadUploads'     => $options['loadUploads'],
    'where'           => $where,
...
//
function _categoryMatchesFormatRules($category, $selectedCategory, $rootDepthVisible, $childDepthVisible, $parentVisibility): bool
{
...
//
function _getListItemEndTags($prevCategory, $thisCategory, $nextCategory): string
{
  $listItemEnd = '';
    ...
    foreach ($fieldnames as $fieldnameString) {
      foreach ($values as $value) {
        ...
        $isDateField = @$fieldSchema['type'] == 'date' ||
                      (@$fieldSchema['type'] == 'none' && ($fieldnamesAsCSV == 'createdDate' || $fieldnamesAsCSV == 'updatedDate'));

        if ($isDateField && strlen((string) $valueAsNumberOnly) == 8) { // add time to dates without it
        ...
        if      ($searchType == '_min')     { $orConditions[] = "$fieldValue+0 >= $valueAsNumberOnly"; }
          ...
          if (@$seenQueries["$fieldnamesAsCSV={$value}_$searchType"]++) { continue; } // only add each query once since we're adding all fields at once
...
//
function _getListDetails($options, $rowCount): array
{
...
function searchMultipleTables($searchTables, $searchOptions) {
  ...
  foreach ($searchTables as $tablename => $tableOptions) {
      foreach (['viewerUrl', 'searchFields', 'titleField'] as $optionName) {
    ...
    if (@$tableOptions['summaryField']) { $subquery .= "`{$tableOptions['summaryField']}` as `_summary`, "; }
      ...
      if (array_key_exists('permalink', $schema)) { $subquery .= "`permalink` as `permalink`, "; }
      else                                             { $subquery .= "'' as `permalink`, "; }
  ...
  if (!empty($searchOptions['orderBy'])) { $query .= " ORDER BY ".$searchOptions['orderBy'] ." "; }
  ...
  if ($searchOptions['keywords'] && $where) {
    if (!empty($searchOptions['debugSql'])) { print "<xmp>$query</xmp>"; }
  ...
  //
  $rows = applyFilters('viewer_output_rows', $rows, $listDetails, []);
...
// returns page number from url.  matches: view.php/anything-here-####.html
function getLastNumberInUrl($defaultNum = null) {
  ...
  foreach ($urlDataFields as $urlData) {
    ...
    $removeFields  = array(
      ...
      'gclid',                                                                    // used by Google Ads auto-tagging: https://support.google.com/analytics/answer/1033981
    );
...
//
function _addWhereConditionsForSpecialFields($schema, $extraConditions, $options = [], $tableName = ''): string
{
  $where            = '';
  $conditions       = [];
  $tableNameDot     = $tableName ? "`$tableName`." : '';

  if ($extraConditions)        {$conditions[] = "($extraConditions)"; }
  if (!@$options['ignoreHidden'] && @$schema['hidden']) {
      $conditions[] = "{$tableNameDot}hidden = 0";
  }
  if (!@$options['ignorePublishDate'] && @$schema['publishDate']) {
      $conditions[] = "{$tableNameDot}publishDate <= NOW()";
  }
  if (!@$options['ignoreRemoveDate'] && @$schema['removeDate'])  {
    ...
    if (!empty($schema['neverRemove'])) { $thisCondition .= " OR {$tableNameDot}neverRemove = 1"; } // never expires checked
...
// return MySQL WHERE clause for Google style query: +word -word "multiple word phrase"
function _getWhereForSearchQuery($query, $fieldnames, $schema = null) {
  ...
  foreach ($matches as $match) {
    [,$plusOrMinus,$quote,$phrase] = $match;
...
//
function _getLink($detailUrl, $filenameFieldValue, $recordNum, $useSeoUrls): string
{
...
function getPrevAndNextRecords($options) {
  ...
  $query .= "  @lastNum := num,\n";                                                                         // get lastRecordNum
  $query .= "  IF(num='$targetNum', (@foundTarget:=1) & (@prevNum:=@lastSeenNum), @lastSeenNum:=num),\n";   // get prevRecordNum (Note that using AND here instead of & caused an issue with unexpected evaluation of the assignment operators on a Windows/MySQL 5.5.33 server)
  ...
  $numsToGet[] = 0;
  [$records, $metaData] = getRecords(array(
    'tableName'   => $options['tableName'],
...
// Note: This function is EXPERIMENTAL and may change in future
function incrementCounterField($tablename, $fieldname, $recordNumber): void
{
    // error checking
    if (!$tablename) {
    ...
    if (!$result) {
        die(__FUNCTION__." MySQL Error: ".htmlencode(mysqli()->error)."\n");
    }
...
function getListLabels($tableName, $fieldName, $fieldValue): array
{
  ...
  $labels = [];
    ...
    if (@$valuesToLabels[$tableName][$fieldName][$value]) {
        $labels[] = $valuesToLabels[$tableName][$fieldName][$value];
    }
    else {
        $labels[] = $value;
        }
