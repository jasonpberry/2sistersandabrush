<?php
// cron.php - command line script to run scheduled background tasks
...
// - were scheduled to run now or since cron.php last executed
...
function cron_dispatcher() { // runs due or overdue jobs
  ...
  foreach (getCronList() as $cron) {
    //print "DEBUG: Checking... " .$cron['functionName']. " => " .$cron['expression']. "\n";

    // get last job run time and oldest time to check
    $jobLastLogRecord  = mysql_get('_cron_log', null, ' `functionName` = "' .mysql_escape($cron['functionName']). '" ORDER BY num DESC');
    ...
    if ($skipTask) {
      if (!inCLI()) { print "Skipping {$cron['activity']}, function: {$cron['functionName']} (not scheduled to run again yet)\n"; }
      continue;
    }

    // Add log entry for job
    $hasLock = mysql_get_lock($cron['functionName']); // get a lock for this specific function
    ...
    if (!inCLI()) { print "Running {$cron['activity']}, function: {$cron['functionName']}\n"; }
    ob_start();
    ...
    $summary   = call_user_func($cron['functionName'], array('note' => 'this $info array is for future use'));
    ...
    mysql_release_lock($cron['functionName']);
  }
...
function cron_logErrorsOnDieOrExit() {
  ...
  if ($secondsAgo >= (60*60)) { // don't email more than once an hour
    ...
    $placeholders = array(
      ...
      'bgtask.function'     => $cronLog['functionName'],
...
function cronExpression_getLastScheduledTime($cronExpression, $oldestTimeToCheck = 0, &$parseErrors = '') {
  ...
  // ... we ask hosts to run cron.php every 1 minute, but we'll assume the worst case of every 90 minutes
...
function cronExpression_getValidValuesForField($type, $expr) {
  ...
  if     ($type == 'minute')     { [$min, $max] = [0, 59];  }
  elseif ($type == 'hour')       { [$min, $max] = [0, 23];  }
  elseif ($type == 'dayOfMonth') { [$min, $max] = [1, 31];  }
  elseif ($type == 'month')      { [$min, $max] = [1, 12];  }
  elseif ($type == 'dayOfWeek')  { [$min, $max] = [0, 6];   } // 0 = Sunday
  ...
  if (preg_match("/^\d+$/", $expr))       { return array((int)$expr); }    // # a specific number
