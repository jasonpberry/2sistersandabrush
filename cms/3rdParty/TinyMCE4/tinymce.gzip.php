...
// START MODIFICATIONS ----------------------------------------------------------------------------
  ...
  // Get cache_dir
  $dataDir        = __DIR__ . "/../../data/"; // default /data/ dir
  $altDataDirFile = __DIR__ . "/../../data_folder.php"; // file which contains alternate datadir location
  if (file_exists($altDataDirFile)) {
    $dataDir = require $altDataDirFile;
    if (!is_dir($dataDir)) { $dataDir = __DIR__ . "/../../" .$dataDir; } // make relative paths absolute
  }
  if (!is_dir($dataDir)) {
    $dataDir_escaped = preg_replace("/\\\\/", "\\\\\\\\", $dataDir); // escape backslashes for display by js
    die("alert('tinymce.gzip.php: couldn\'t find cms data dir:\\n$dataDir_escaped');");
  }
  $GLOBALS['TINYMCE_CACHE_DIR'] = realpath($dataDir);

// - Original source file can be found here: https://github.com/tinymce/tinymce_compressor/blob/master/tinymce.gzip.php
// - Changed cache_dir to be $GLOBALS['TINYMCE_CACHE_DIR']
...
class TinyMCE_Compressor {
   ...
   */
    ...
    self::$defaultSettings['cache_dir'] = $GLOBALS['TINYMCE_CACHE_DIR'];                           // CUSTOM CHANGES
